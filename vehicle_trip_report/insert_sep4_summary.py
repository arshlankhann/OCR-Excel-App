"""
Insert Sep 4 Manual Summary Data into vehicle_trips
=====================================================
Data provided:
  J.P Bros  = 307580 kg, 16 trips  -> Pratapgarh
  SVN       = 389000 kg, 22 trips  -> Pratapgarh

Usage:
    python insert_sep4_summary.py              # insert (skip if date exists)
    python insert_sep4_summary.py --overwrite  # delete & re-insert
    python insert_sep4_summary.py --dry-run    # preview only
"""
...
TRIP_DATE   = "04-09-2026"
TRIP_TIME   = "08:00:00"
SLIP_TYPE   = "Input"

# (agency_name, total_weight_kg, num_trips, processing_unit, reported_by)
SEP4_DATA = [
    ("J.P Bros",   307580, 16, "Pratapgarh", "Krishan JE"),
    ("SVN",        389000, 22, "Pratapgarh", "Krishan JE"),
    ("V S Waste",  138730, 10, "Mujeri", "V S Waste"),
    ("Tractor",     32450, 30, "Mujeri", "Tractor"),
]


def build_records():
    records = []
    rst_counter = 96000  # "M" prefix + high number to avoid clashes
    for agency, total_wt, num_trips, unit, reporter in SEP4_DATA:
...
def run(dry_run=False, overwrite=False):
    ...
    print(f"\n{'DRY RUN - ' if dry_run else ''}Sep 4 Summary Import")
    print("=" * 60)
    print(f"\n{'Agency':<15} {'Unit':<12} {'Trips':>6} {'Total Wt (kg)':>14}")
    print("-" * 50)
    for agency, total_wt, num_trips, unit, _ in SEP4_DATA:
        print(f"{agency:<15} {unit:<12} {num_trips:>6} {total_wt:>14,}")
    print("-" * 50)
    print(f"{'TOTAL':<15} {'':<12} {sum(x[2] for x in SEP4_DATA):>6} {sum(x[1] for x in SEP4_DATA):>14,}")
