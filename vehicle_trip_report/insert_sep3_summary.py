"""
Insert Sep 3 Manual Summary Data into vehicle_trips
=====================================================
Data provided:
  V S Waste = 71260 kg, 5 trips  -> Mujeri
  Tractor   = 39670 kg, 40 trips -> Mujeri

Usage:
    python insert_sep3_summary.py              # insert (skip if date exists)
    python insert_sep3_summary.py --overwrite  # delete & re-insert
    python insert_sep3_summary.py --dry-run    # preview only
"""
...
TRIP_DATE   = "03-09-2026"
TRIP_TIME   = "08:00:00"
SLIP_TYPE   = "Input"

# (agency_name, total_weight_kg, num_trips, processing_unit, reported_by)
SEP3_DATA = [
    ("V S Waste",  71260,  5, "Mujeri", "V S Waste"),
    ("Tractor",    39670, 40, "Mujeri", "Tractor"),
]


def build_records():
    records = []
    rst_counter = 95000  # "M" prefix + high number to avoid clashes
    for agency, total_wt, num_trips, unit, reporter in SEP3_DATA:
...
def run(dry_run=False, overwrite=False):
    ...
    print(f"\n{'DRY RUN - ' if dry_run else ''}Sep 3 Summary Import")
    print("=" * 60)
    print(f"\n{'Agency':<15} {'Unit':<12} {'Trips':>6} {'Total Wt (kg)':>14}")
    print("-" * 50)
    for agency, total_wt, num_trips, unit, _ in SEP3_DATA:
        print(f"{agency:<15} {unit:<12} {num_trips:>6} {total_wt:>14,}")
    print("-" * 50)
    print(f"{'TOTAL':<15} {'':<12} {sum(x[2] for x in SEP3_DATA):>6} {sum(x[1] for x in SEP3_DATA):>14,}")
