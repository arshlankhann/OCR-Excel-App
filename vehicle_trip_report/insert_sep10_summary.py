"""
Insert Sep 10 Manual Summary Data into vehicle_trips
=====================================================
Data provided:
  J.P Bros   = 114060 kg,  5 trips  -> Pratapgarh

Usage:
    python insert_sep10_summary.py              # insert (skip if date exists)
    python insert_sep10_summary.py --overwrite  # delete & re-insert
    python insert_sep10_summary.py --dry-run    # preview only
"""
...
TRIP_DATE   = "10-09-2026"
TRIP_TIME   = "08:00:00"
SLIP_TYPE   = "Input"

# (agency_name, total_weight_kg, num_trips, processing_unit, reported_by)
SEP10_DATA = [
    ("J.P Bros",   114060,  5, "Pratapgarh", "Krishan JE"),
    ("Government",  16890,  2, "Mujeri", "Krishan JE"),
    ("SVN",        228990, 14, "Mujeri", "Krishan JE"),
    ("Tractor",     29880, 22, "Mujeri", "Tractor"),
    ("V S Waste",  148860, 10, "Mujeri", "V S Waste"),
]


def build_records():
    records = []
    rst_counter = 102000  # "M" prefix + high number to avoid clashes
    for agency, total_wt, num_trips, unit, reporter in SEP10_DATA:
...
def run(dry_run=False, overwrite=False):
    ...
    print(f"\n{'DRY RUN - ' if dry_run else ''}Sep 10 Summary Import")
    print("=" * 60)
    print(f"\n{'Agency':<15} {'Unit':<12} {'Trips':>6} {'Total Wt (kg)':>14}")
    print("-" * 50)
    for agency, total_wt, num_trips, unit, _ in SEP10_DATA:
        print(f"{agency:<15} {unit:<12} {num_trips:>6} {total_wt:>14,}")
    print("-" * 50)
    print(f"{'TOTAL':<15} {'':<12} {sum(x[2] for x in SEP10_DATA):>6} {sum(x[1] for x in SEP10_DATA):>14,}")
