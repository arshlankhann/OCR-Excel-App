"""
Insert Sep 11 Manual Summary Data into vehicle_trips
=====================================================
Data provided:
  J.P Bros   = 129830 kg,  6 trips  -> Pratapgarh

Usage:
    python insert_sep11_summary.py              # insert (skip if date exists)
    python insert_sep11_summary.py --overwrite  # delete & re-insert
    python insert_sep11_summary.py --dry-run    # preview only
"""
...
SEP11_DATA = [
    ...
    ("SVN",        162130, 10, "Mujeri", "Krishan JE"),
...
def build_records():
    records = []
    rst_counter = 103000  # "M" prefix + high number to avoid clashes
...
def run(dry_run=False, overwrite=False):
    ...
    print(f"\n{'DRY RUN - ' if dry_run else ''}Sep 11 Summary Import")
    print("=" * 60)
    ...
    print("-" * 50)
    print(f"{'TOTAL':<15} {'':<12} {sum(x[2] for x in SEP11_DATA):>6} {sum(x[1] for x in SEP11_DATA):>14,}")
