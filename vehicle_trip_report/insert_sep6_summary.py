"""
Insert Sep 6 Manual Summary Data into vehicle_trips
=====================================================
Data provided:
  J.P Bros   = 373760 kg, 18 trips  -> Pratapgarh
  SVN        = 548690 kg, 31 trips  -> Pratapgarh

Usage:
    python insert_sep6_summary.py              # insert (skip if date exists)
    python insert_sep6_summary.py --overwrite  # delete & re-insert
    python insert_sep6_summary.py --dry-run    # preview only
"""
...
TRIP_DATE   = "06-09-2026"
TRIP_TIME   = "08:00:00"
SLIP_TYPE   = "Input"

# (agency_name, total_weight_kg, num_trips, processing_unit, reported_by)
SEP6_DATA = [
    ("J.P Bros",   373760, 18, "Pratapgarh", "Krishan JE"),
    ("SVN",        548690, 31, "Pratapgarh", "Krishan JE"),
    ("Tractor",     26330, 25, "Mujeri", "Tractor"),
    ("V S Waste",  128500,  9, "Mujeri", "V S Waste"),
]


def build_records():
    records = []
    rst_counter = 98000  # "M" prefix + high number to avoid clashes
    for agency, total_wt, num_trips, unit, reporter in SEP6_DATA:
...
def run(dry_run=False, overwrite=False):
    ...
    print(f"\n{'DRY RUN - ' if dry_run else ''}Sep 6 Summary Import")
    print("=" * 60)
    print(f"\n{'Agency':<15} {'Unit':<12} {'Trips':>6} {'Total Wt (kg)':>14}")
    print("-" * 50)
    for agency, total_wt, num_trips, unit, _ in SEP6_DATA:
        print(f"{agency:<15} {unit:<12} {num_trips:>6} {total_wt:>14,}")
    print("-" * 50)
    print(f"{'TOTAL':<15} {'':<12} {sum(x[2] for x in SEP6_DATA):>6} {sum(x[1] for x in SEP6_DATA):>14,}")
