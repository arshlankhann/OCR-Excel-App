"""
Insert Sep 7 Manual Summary Data into vehicle_trips
=====================================================
Data provided:
  J.P Bros   = 108470 kg,  5 trips  -> Pratapgarh
  SVN        =  40810 kg,  2 trips  -> Pratapgarh

Usage:
    python insert_sep7_summary.py              # insert (skip if date exists)
    python insert_sep7_summary.py --overwrite  # delete & re-insert
    python insert_sep7_summary.py --dry-run    # preview only
"""
...
TRIP_DATE   = "07-09-2026"
TRIP_TIME   = "08:00:00"
SLIP_TYPE   = "Input"

# (agency_name, total_weight_kg, num_trips, processing_unit, reported_by)
SEP7_DATA = [
    ("J.P Bros",   108470,  5, "Pratapgarh", "Krishan JE"),
    ("SVN",         40810,  2, "Pratapgarh", "Krishan JE"),
    ("J.P Bros",    17870,  1, "Mujeri", "Krishan JE"),
    ("SVN",        124180,  8, "Mujeri", "Krishan JE"),
    ("Tractor",     19990, 17, "Mujeri", "Tractor"),
    ("V S Waste",  148580, 10, "Mujeri", "V S Waste"),
]


def build_records():
    records = []
    rst_counter = 99000  # "M" prefix + high number to avoid clashes
    for agency, total_wt, num_trips, unit, reporter in SEP7_DATA:
...
def run(dry_run=False, overwrite=False):
    ...
    print(f"\n{'DRY RUN - ' if dry_run else ''}Sep 7 Summary Import")
    print("=" * 60)
    print(f"\n{'Agency':<15} {'Unit':<12} {'Trips':>6} {'Total Wt (kg)':>14}")
    print("-" * 50)
    for agency, total_wt, num_trips, unit, _ in SEP7_DATA:
        print(f"{agency:<15} {unit:<12} {num_trips:>6} {total_wt:>14,}")
    print("-" * 50)
    print(f"{'TOTAL':<15} {'':<12} {sum(x[2] for x in SEP7_DATA):>6} {sum(x[1] for x in SEP7_DATA):>14,}")
