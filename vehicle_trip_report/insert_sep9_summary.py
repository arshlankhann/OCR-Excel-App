"""
Insert Sep 9 Manual Summary Data into vehicle_trips
=====================================================
Data provided:
  J.P Bros   = 136070 kg,  6 trips  -> Pratapgarh

Usage:
    python insert_sep9_summary.py              # insert (skip if date exists)
    python insert_sep9_summary.py --overwrite  # delete & re-insert
    python insert_sep9_summary.py --dry-run    # preview only
"""
...
TRIP_DATE   = "09-09-2026"
TRIP_TIME   = "08:00:00"
SLIP_TYPE   = "Input"

# (agency_name, total_weight_kg, num_trips, processing_unit, reported_by)
SEP9_DATA = [
    ("J.P Bros",   136070,  6, "Pratapgarh", "Krishan JE"),
    ("SVN",        231730, 14, "Mujeri", "Krishan JE"),
    ("Tractor",    135800, 71, "Mujeri", "Tractor"),
    ("V S Waste",  141290, 10, "Mujeri", "V S Waste"),
]


def build_records():
    records = []
    rst_counter = 101000  # "M" prefix + high number to avoid clashes
    for agency, total_wt, num_trips, unit, reporter in SEP9_DATA:
...
def run(dry_run=False, overwrite=False):
    ...
    print(f"\n{'DRY RUN - ' if dry_run else ''}Sep 9 Summary Import")
    print("=" * 60)
    print(f"\n{'Agency':<15} {'Unit':<12} {'Trips':>6} {'Total Wt (kg)':>14}")
    print("-" * 50)
    for agency, total_wt, num_trips, unit, _ in SEP9_DATA:
        print(f"{agency:<15} {unit:<12} {num_trips:>6} {total_wt:>14,}")
    print("-" * 50)
    print(f"{'TOTAL':<15} {'':<12} {sum(x[2] for x in SEP9_DATA):>6} {sum(x[1] for x in SEP9_DATA):>14,}")
